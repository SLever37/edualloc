    ...
    <div className="space-y-8 animate-in fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm">
          ...
          <form 
            onSubmit={(e) => { e.preventDefault(); if(novoSetor.trim()) { aoAdicionarSetor(novoSetor); setNovoSetor(''); } }} 
          ...
          >
            ...
            <button 
                ...
                className="px-6 h-12 bg-indigo-600 text-white rounded-xl font-black text-xs uppercase hover:bg-indigo-700 transition shadow-lg shadow-indigo-100 disabled:opacity-50 disabled:cursor-not-allowed"
            >
                Add
            </button>
          </form>
          <div className="space-y-2 max-h-96 overflow-y-auto custom-scrollbar pr-2">
            {setores.length === 0 && <p className="text-xs text-slate-400 italic text-center py-4">Nenhum setor cadastrado.</p>}
            {setores.map(s => (
                ...
                {editSetorId === s.id ? (
                    <div className="flex flex-1 items-center gap-2">
                        <input 
                            autoFocus
                            type="text" 
                            className="flex-1 h-8 px-2 bg-white border border-indigo-300 rounded-lg text-sm font-bold text-slate-800 outline-none"
                            value={editSetorNome}
                            onChange={e => setEditSetorNome(e.target.value)}
                            onKeyDown={e => { if(e.key === 'Enter') saveSetor(s.id); if(e.key === 'Escape') cancelSetor(); }}
                        />
                    ...
                    <>
                        ...
                        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition">
                            <button onClick={() => startEditSetor(s)} className="text-indigo-400 hover:text-indigo-600 p-1 hover:bg-indigo-50 rounded-lg" title="Editar"><svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path d="M17 3a2.85 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"/><path d="m15 5 4 4"/></svg></button>
                            <button onClick={() => aoRemoverSetor(s.id)} className="text-rose-400 hover:text-rose-600 p-1 hover:bg-rose-50 rounded-lg" title="Remover"><svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path d="M3 6h18M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6m3 0V4a2 2 0 012-2h4a2 2 0 012 2v2"/></svg></button>
                        </div>
                    </>
                )}
              </div>
        ...
        <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm">
          ...
          <form 
            onSubmit={(e) => { e.preventDefault(); if(novaFuncao.trim()) { aoAdicionarFuncao(novaFuncao); setNovaFuncao(''); } }}
            className="flex gap-3 mb-6"
          >
            <input 
              type="text" 
            ...
            <button 
                ...
                className="px-6 h-12 bg-emerald-600 text-white rounded-xl font-black text-xs uppercase hover:bg-emerald-700 transition shadow-lg shadow-emerald-100 disabled:opacity-50 disabled:cursor-not-allowed"
            >
                Add
            </button>
          </form>
          <div className="space-y-2 max-h-96 overflow-y-auto custom-scrollbar pr-2">
            {funcoes.length === 0 && <p className="text-xs text-slate-400 italic text-center py-4">Nenhuma função cadastrada.</p>}
            {funcoes.map(f => (
                ...
                {editFuncaoId === f.id ? (
                    <div className="flex flex-1 items-center gap-2">
                        <input 
                            autoFocus
                            type="text" 
                            className="flex-1 h-8 px-2 bg-white border border-emerald-300 rounded-lg text-sm font-bold text-slate-800 outline-none"
                            value={editFuncaoNome}
                            onChange={e => setEditFuncaoNome(e.target.value)}
                            onKeyDown={e => { if(e.key === 'Enter') saveFuncao(f.id); if(e.key === 'Escape') cancelFuncao(); }}
                        />
                    ...
                    <>
                        ...
                        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition">
                            <button onClick={() => startEditFuncao(f)} className="text-emerald-400 hover:text-emerald-600 p-1 hover:bg-emerald-50 rounded-lg" title="Editar"><svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path d="M17 3a2.85 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"/><path d="m15 5 4 4"/></svg></button>
                            <button onClick={() => aoRemoverFuncao(f.id)} className="text-rose-400 hover:text-rose-600 p-1 hover:bg-rose-50 rounded-lg" title="Remover"><svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path d="M3 6h18M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6m3 0V4a2 2 0 012-2h4a2 2 0 012 2v2"/></svg></button>
                        </div>
                    </>
                )}
              </div>
