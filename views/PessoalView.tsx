...
const PessoalView: React.FC<PessoalViewProps> = ({ funcionarios, escolas, funcoes, setores, aoEditar, aoAdicionar, aoRemover, onImport }) => {
  ...
  const handleWhatsApp = (f: Funcionario) => {
    // Se tiver telefone cadastrado, usa ele. Se não, pergunta.
    let tel = f.telefone;
    ...
    if (tel) {
        ...
        window.open(`https://wa.me/${cleanTel}?text=Olá ${f.nome}, mensagem do RH.`, '_blank');
    }
  };

  const handleEmail = (f: Funcionario) => {
    // Se tiver email cadastrado, abre direto.
    const recipient = f.email ? f.email : "";
    window.open(`mailto:${recipient}?subject=Aviso RH - ${f.nome}&body=Prezado(a) ${f.nome},`, '_blank');
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-2xl md:text-3xl font-black text-slate-800 tracking-tight">Pessoal Global</h2>
          <p className="text-sm md:text-base text-slate-500">Gestão completa de todos os profissionais da rede.</p>
        </div>
        <div className="flex gap-3 w-full md:w-auto">
          {onImport && (
            <button onClick={() => setShowImport(true)} className="flex-1 md:flex-none px-4 py-3 bg-white border border-slate-200 text-indigo-600 rounded-xl font-black text-xs uppercase hover:bg-slate-50 transition shadow-sm whitespace-nowrap">
                Importar Excel
            </button>
          )}
          <button onClick={aoAdicionar} className="flex-1 md:flex-none px-6 py-3 bg-indigo-600 text-white rounded-xl font-black shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition whitespace-nowrap">
            + Novo
          </button>
        </div>
      </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredFuncionarios.map(f => (
                <tr key={f.id} className="hover:bg-slate-50 transition relative">
                  <td className="px-6 py-4">
                    ...
                    {(f.telefone || f.email) && (
                        <div className="flex gap-2 mt-1">
                            {f.telefone && <span className="text-[9px] bg-emerald-50 text-emerald-700 px-1.5 py-0.5 rounded font-bold" title={f.telefone}>TEL</span>}
                            {f.email && <span className="text-[9px] bg-sky-50 text-sky-700 px-1.5 py-0.5 rounded font-bold" title={f.email}>EMAIL</span>}
                        </div>
                    )}
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    <p className="text-sm font-bold text-slate-600 truncate max-w-[200px]">{escolas.find(e => e.id === f.escolaId)?.nome || 'Não Alocado'}</p>
                  </td>
                  <td className="px-6 py-4">
                            ...
                            {openActionId === f.id && (
                                <>
                                    ...
                                    <div className="absolute right-0 top-full mt-2 w-48 bg-white rounded-xl shadow-xl border border-slate-100 z-20 p-2 animate-in fade-in zoom-in-95 origin-top-right">
                                        ...
                                        <button onClick={() => { handleWhatsApp(f); setOpenActionId(null); }} className="flex items-center gap-3 w-full p-2 hover:bg-emerald-50 text-slate-600 hover:text-emerald-700 rounded-lg text-xs font-bold transition text-left mb-1">
                                            <div className="w-6 h-6 rounded-full bg-emerald-100 flex items-center justify-center text-emerald-600">
                                                <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/></svg>
                                            </div>
                                            WhatsApp {f.telefone && <span className="text-[9px] text-emerald-600 opacity-60 ml-auto">✓</span>}
                                        </button>
                                        <button onClick={() => { handleEmail(f); setOpenActionId(null); }} className="flex items-center gap-3 w-full p-2 hover:bg-sky-50 text-slate-600 hover:text-sky-700 rounded-lg text-xs font-bold transition text-left">
                                            ...
                                            E-mail {f.email && <span className="text-[9px] text-sky-600 opacity-60 ml-auto">✓</span>}
                                        </button>
                                    </div>
                                </>
                            )}
