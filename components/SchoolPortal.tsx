...
const SchoolPortal: React.FC<SchoolPortalProps> = ({ 
  ...
  const handleWhatsApp = (f: Funcionario) => {
    ...
    if (tel) {
        ...
        window.open(`https://wa.me/${cleanTel}?text=Olá ${f.nome}, mensagem da gestão da unidade ${school.nome}.`, '_blank');
    } else {
        alert("Número de WhatsApp não cadastrado para este servidor.");
    }
  };
  ...
  return (
    <div className="space-y-6 md:space-y-8 animate-in fade-in duration-500 pb-20">
      ...
      {/* Header com Logo e Identidade da Escola */}
      <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-200 overflow-hidden">
        <div className="p-8 md:p-10 bg-slate-900 text-white flex flex-col md:flex-row items-center gap-8 relative overflow-hidden">
          ...
          <div className="text-center md:text-left flex-1 relative z-10">
            ...
            <div className="flex flex-wrap justify-center md:justify-start gap-4 text-xs font-bold text-slate-400">
               ...
               <span className="flex items-center gap-2 px-3 py-1 bg-white/5 rounded-full border border-white/10 uppercase tracking-widest text-[10px]">Código: {school.codigoGestor}</span>
            </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 border-t border-slate-100 divide-x divide-slate-100">
          ...
          <div className="p-6 text-center">
            <p className="text-[10px] font-black text-emerald-500 uppercase tracking-widest mb-1">Confirmados Hoje</p>
            <p className="text-2xl font-black text-emerald-600">{presentes}</p>
          </div>
          <div className="p-6 text-center">
            <p className="text-[10px] font-black text-rose-500 uppercase tracking-widest mb-1">Faltantes</p>
            <p className="text-2xl font-black text-rose-600">{ausentesHoje}</p>
          </div>
          <div className="p-6 text-center">
            <p className="text-[10px] font-black text-indigo-500 uppercase tracking-widest mb-1">Status Rede</p>
            <div className="flex gap-1 justify-center mt-1">
               <span className="px-2 py-0.5 bg-indigo-50 text-indigo-600 rounded text-[9px] font-black uppercase">Sincronizado</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        {/* Listagem de Servidores por Setor */}
        <div className="xl:col-span-2 space-y-6">
          {Object.entries(groupedBySector).map(([sector, members]) => (
            <div key={sector} className="bg-white rounded-[2rem] border border-slate-200 overflow-hidden shadow-sm">
              ...
              <div className="divide-y divide-slate-100">
                {(members as any[]).map(emp => (
                  <div key={emp.id} className="p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 hover:bg-slate-50 transition-all group">
                    <div className="flex gap-5 items-center min-w-0 flex-1">
                      <div className="w-14 h-14 rounded-2xl bg-slate-100 border border-slate-200 flex-shrink-0 overflow-hidden flex items-center justify-center">
                        {emp.fotoUrl ? (
                          <img src={emp.fotoUrl} className="w-full h-full object-cover" />
                        ) : (
                          <div className="text-slate-300 font-black text-xl">{emp.nome.charAt(0)}</div>
                        )}
                      </div>
                      <div className="min-w-0">
                        ...
                        {emp.status !== StatusFuncionario.ATIVO && (
                          <span className="mt-1 inline-block px-1.5 py-0.5 bg-amber-100 text-amber-700 rounded text-[8px] font-black uppercase">{emp.status}</span>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3 w-full md:w-auto">
                      <button 
                        ...
                        className="p-3 bg-emerald-50 text-emerald-600 border border-emerald-100 rounded-xl hover:bg-emerald-600 hover:text-white transition-all shadow-sm"
                      ...
                      >
                        {emp.presencaConfirmada ? 'Presente' : 'Registrar'}
                      </button>
                    </div>
                  </div>
                ))}
        ...
        <div className="space-y-6">
          <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm sticky top-8">
            ...
            <textarea 
              readOnly={!editNotes}
              className={`w-full h-40 p-5 rounded-2xl text-sm font-medium outline-none transition-all resize-none ${editNotes ? 'bg-slate-50 border-2 border-indigo-500' : 'bg-indigo-50/30 border-transparent text-indigo-900 cursor-default'}`}
              placeholder="Digite um aviso para o RH ou equipe..."
              value={tempNotes}
            ...
            >
                {editNotes ? 'Salvar Aviso' : 'Editar Notas Internas'}
            </button>
            
            <div className="mt-8 pt-8 border-t border-slate-100">
               <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">Contatos de Suporte RH</h4>
               <div className="space-y-3">
                  {rhContacts.map((c, idx) => (
                      <div key={idx} className="p-3 bg-slate-50 rounded-xl border border-slate-100 flex items-center gap-3">
                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${c.type === 'email' ? 'bg-sky-100 text-sky-600' : 'bg-emerald-100 text-emerald-600'}`}>
                           {c.type === 'email' ? <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><rect width="20" height="16" x="2" y="4" rx="2"/><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"/></svg> : <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/></svg>}
                        </div>
                        <div className="min-w-0">
                           <p className="text-[9px] font-black text-slate-800 uppercase truncate">{c.label}</p>
                           <p className="text-[11px] font-bold text-slate-500 truncate">{c.value}</p>
                        </div>
                      </div>
                  ))}
               </div>
      ...
      {modalFalta && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-[110] flex items-center justify-center p-4">
          <div className="bg-white rounded-[2.5rem] w-full max-w-md p-8 shadow-2xl animate-modal">
            ...
            <p className="text-sm text-slate-500 mb-4">Servidor: <strong>{modalFalta.nome}</strong></p>
            <textarea 
              ...
              placeholder="Justificativa ou motivo da falta..."
              value={obsFalta}
              onChange={e => setObsFalta(e.target.value)}
            />
            <div className="flex gap-4">
              <button onClick={() => setModalFalta(null)} className="flex-1 py-4 font-black text-[10px] text-slate-500 uppercase">Cancelar</button>
              <button onClick={() => { onToggleAttendance(modalFalta.id, OcorrenciaFrequencia.FALTA_INJUSTIFICADA, obsFalta); setModalFalta(null); }} className="flex-1 py-4 bg-rose-600 text-white rounded-xl font-black text-[10px] uppercase shadow-lg shadow-rose-200">Confirmar Falta</button>
            </div>
          </div>
        </div>
      )}
