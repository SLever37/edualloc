      ...
      `}>
        ...
        <nav className="flex-1 p-4 space-y-2 mt-2 overflow-y-auto custom-scrollbar">
          {(user.perfil === Perfil.ADMINISTRADOR || user.perfil === Perfil.SUPER_ADMIN) ? (
            <>
              <button onClick={() => handleNavigation('dashboard')} className={`w-full text-left p-3 rounded-xl transition font-bold text-sm flex items-center gap-3 ${activeView === 'dashboard' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}>Painel Principal</button>
              <button onClick={() => handleNavigation('employees')} className={`w-full text-left p-3 rounded-xl transition font-bold text-sm flex items-center gap-3 ${activeView === 'employees' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}>Pessoal Global</button>
              <button onClick={() => handleNavigation('schools')} className={`w-full text-left p-3 rounded-xl transition font-bold text-sm flex items-center gap-3 ${activeView === 'schools' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}>Gestão de Escolas</button>
              <button onClick={() => handleNavigation('settings')} className={`w-full text-left p-3 rounded-xl transition font-bold text-sm flex items-center gap-3 ${activeView === 'settings' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}>Configurações</button>
            ...
            <button onClick={() => handleNavigation('portal-escola')} className={`w-full text-left p-3 rounded-xl transition font-bold text-sm flex items-center gap-3 ${activeView === 'portal-escola' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}>Minha Unidade</button>
          )}
        </nav>
        ...
        <div className="p-4 border-t border-slate-800 bg-slate-900/50 shrink-0 space-y-2 safe-area-bottom">
          ...
          >
            ...
            <div className="overflow-hidden text-left min-w-0">
              <p className="text-sm font-black truncate leading-tight uppercase tracking-tighter">{user.nome.split(' ')[0]}</p>
              <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest truncate">Editar Perfil</p>
            </div>
          </button>
          <button onClick={handleLogout} className="w-full flex items-center gap-3 p-3 rounded-xl text-rose-400 hover:bg-rose-500/10 hover:text-rose-300 transition group cursor-pointer">
